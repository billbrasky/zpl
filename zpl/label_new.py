...
class Block:
    def __int__( self, x, y, justification ):
        self.x = x
        self.y = y
...
class Label:
    ...
    def wrapBlock( self, x, y, justification, block ):
        """
        Wraps the provided block in an ^FO and ^FS 'tag'py
        """
        if self.code == '^XA':
            return

        self.code += f'^FO{x*self.dpmm},{y*self.dpmm}'
    ...
    def textblock(
        ...
        """
        assert text_justification in ['L','R','C','J']
        block = f'^FB{width * self.dpmm},{lines},{0},{text_justification},{0}'
        self.wrapBlock( x, y, block_justification, block )
    ...
    def write_text(
        ...
        if char_height and char_width and font and orientation:
            ...
            if re.match(r'^[A-Z0-9]$', font):
                block += "^A%c%c,%i,%i" % (font, orientation, char_height*self.dpmm,
                                               char_width*self.dpmm)
            elif re.match(r'[REBA]?:[A-Z0-9\_]+\.(FNT|TTF|TTE)', font):
                block += "^A@%c,%i,%i,%s" % (orientation, char_height*self.dpmm,
                                               char_width*self.dpmm, font)
            else:
                raise ValueError("Invalid font.")
        if line_width:
            assert justification in "LCRJ", "invalid justification"
            block += "^FB%i,%i,%i,%c,%i" % (line_width*self.dpmm, max_line, line_spaces,
                                                justification, hanging_indent)
        block += "^FD%s" % text

        self.wrap( x, y, block_justification, block )
